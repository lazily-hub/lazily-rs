...
EXPECTED_GATE_STEPS=(
	...
	"test-durable-client	Test lightweight durable client against canonical spec fixtures"
	"test-durable-outbox	Test durable outbox store protocol against canonical spec fixtures"
	"test-durable-postgres	Test PostgreSQL durable owner transaction and relay recovery"
	"test-durable-jetstream	Test NATS JetStream durable transport crash windows"
