...
/// Run a fixture whose emit projection is a `CellHandle<Option<String>>`, given a
/// per-step driver returning the emitted value and the current output.
fn run<F>(ctx: &Context, fx: &Value, observed: lazily::SlotHandle<Option<String>>, mut drive: F)
where
