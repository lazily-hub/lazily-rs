...
// -- ThreadSafeContext / AsyncContext --------------------------------------
//
// Both are added alongside their `dispose_slot`/`dispose_cell`/`scope()`
// implementations. The corpus is parameterised over the model precisely so that
// adding a model is a matter of writing an impl, not of rewriting the engine.
