...
use lazily::{
    BackpressurePolicy, BoundDim, Context, Max, MergeCellHandle, MergePolicy, Overflow, RelayCell,
    SetUnion, SpillMode, SpillStore, Sum,
};
...
fn relay_spills_full_windows_losslessly() {
    ...
    let egress: MergeCellHandle<i64, Sum> = ctx.merge_cell(0);
