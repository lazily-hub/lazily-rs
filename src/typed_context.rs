...
/// factory functions generated by `#[lazily::slot]` / `#[lazily::cell]`.
...
{
    fn set_typed(self, ctx: &TypedContext<Schema>, value: T) {
        ctx.inner.set_cell(&self.raw, value);
    }
}
    ...
    F: Fn(&TypedContext<Schema>) -> TypedCellHandle<Schema, T>,
{
    fn set_typed(self, ctx: &TypedContext<Schema>, value: T) {
        let handle = self(ctx);
        ctx.inner.set_cell(&handle.raw, value);
    }
}
