...
- `src/materialization.rs` — materialization mode (`#lzmatmode`): `MaterializationMode` (eager default / lazy opt-in) + `MaterializedFamily<K, V>`, a keyed derived-slot family that materializes every key up front (eager) or on first read (lazy). Observationally transparent — a keyed overlay on the eager `Context` core (`ctx.memo` slots), not a second engine. Conformance: `tests/materialization_conformance.rs`
...
- `tests/materialization_conformance.rs` — materialization mode compute fixtures (lazily-spec/conformance/materialization/`*.json`); observational transparency (eager vs. lazy identical `observe`), eager-materializes-all vs. lazy-defers, monotone present set + subset-of-eager, default mode eager (`#lzmatmode`)
