...
- `src/reactive_graph.rs` — capability traits over the three execution models (`#lzspecedgeindex`): bound-free `ReactiveGraph` (disposal, scopes, batch, degree introspection, associated `FormulaCell`/`SourceCell`/`EffectHandle`/`Scope` types), `Teardown` (scope contract), `SyncReactiveGraph`/`AsyncReactiveGraph` (construction + reads, split by read discipline), and the blanket-implemented `ThreadSafeReactiveGraph` marker. Read discipline and thread-safety are orthogonal axes, hence four traits
- `src/context.rs` — `Context` struct, dependency graph, thread-local tracking stack
- `src/cell.rs` — **the Cell kernel** (`#lzcellkernel`): the genus `Cell<T, K>` (a lightweight `Copy` `SlotId` into Context) with kind markers `Source<M>` / `Formula`, and the aliases `SourceCell<T, M = KeepLatest>` = `Cell<T, Source<M>>` and `FormulaCell<T>` = `Cell<T, Formula>`. Reads (`get`, `subscribe`) live on `impl<T, K> Cell<T, K>`; writes (`set`, `merge`) live on `impl<T, M: MergePolicy<T>> Cell<T, Source<M>>`, so `formula.set(…)` is a *"no method"* compile error (write protection without a trait, design §3). Formula lifecycle (`drive`/`undrive`/`is_driven`/`clear`/`get_rc`) is on `impl<T> Cell<T, Formula>`. Replaces the former `SlotHandle`/`CellHandle`/`SignalHandle`/`MergeCellHandle` structs (all deleted) and the vestigial `Reactive<T>`/`Source<T>` traits (deleted). Note `src/slot.rs` and `src/signal.rs` are gone — `Slot` survives only as the *storage* sense (`SlotId`, `SlotNode` in `context.rs`)
- `src/cell_family.rs` — `ReactiveMap<K, V, H>` unified keyed reactive collection (`#reactivemap`) over the `MapHandle` trait (`SourceCell` input cells / `FormulaCell` derived formulas) + `EntryKind`. `CellMap<K,V> = ReactiveMap<K,V,SourceCell<V>>` (adds cell-only `set` + eager value-minting `entry`/`entry_with`); `SlotMap<K,V> = ReactiveMap<K,V,FormulaCell<V>>` (lazy `get_or_insert_with` mint-on-access + eager `materialize_all` pre-mint; no `set`). `MapHandle` is **kept** (not collapsed): its two impls are for the distinct instantiations `Cell<V, Source<KeepLatest>>` and `Cell<V, Formula>`, carrying `const KIND: EntryKind` per kind. No eager/lazy mode flag — eager = pre-mint loop, lazy = mint-on-access
...
- **Eager construction** (`#lzcellkernel`, formerly `src/signal.rs`) — a driven `FormulaCell`: `ctx.formula(f).drive()` (or the `ctx.signal(f)` convenience) builds a guarded formula and attaches a puller `Effect` that keeps it materialized. `.drive()` is idempotent and returns the *same* handle; `.undrive()` reverses it. Drivenness is graph state — a `driven` bit on `SlotNode` plus the `driven_by: HashMap<SlotId, SlotId>` side table in `context.rs` (cleared on dispose/undrive) — not a distinct type, so the former `Signal`/`SignalHandle` are retired and the `#lzsignaleager` per-write-puller bug is structurally unwritable. `ThreadSafeContext`/`AsyncContext` keep their own signal handles for now
...
- `src/merge.rs` — RelayCell Phase 1 (`#relaycell`): the `MergePolicy` merge algebra (associative `⊕`, `const COMMUTATIVE`/`IDEMPOTENT` flags) and concrete policies (`KeepLatest`/`Sum`/`Max`/`SetUnion`/`RawFifo`, plus `CrdtJoin<C>` wiring existing `CellCrdt` units behind `distributed`). Under the Cell kernel (`#lzcellkernel`) a "merge cell" is just a `SourceCell<T, M>` (`Cell<T, Source<M>>`) with `M ≠ KeepLatest`; the identity `Cell ≡ MergeCell<KeepLatest>` is a type alias with a default parameter, not a spec assertion. The former `MergeCellHandle<T,M>` struct and the `Reactive<T>`/`Source<T>: Reactive<T>` read/write traits are **deleted**. Constructors `Context::source::<M>(v)` (was `merge_cell`) / `Context::apply_merge` (the merge write by id, routes through `set_source`).
...
- `src/rateshape.rs` — rate-shaping source operators (`#lzrateshape`): the lifted `RatePolicy`/`WindowPolicy`/`ExpiryPolicy` (moved out of `relay_policy`; re-exported at crate top-level so relay semantics/API are unchanged) plus source-level operators over any `Cell<T, K>` — `DebounceCell`/`Core` (emit latest after quiet period), `ThrottleCell`/`Core` + `ThrottleEdge` (Leading/Trailing, one emit per window), `SampleCell`/`Core` + `SampleMode` (Count every-nth / Time every-boundary), `ProbabilisticSampleCell`/`Core` + `SampleRng`/`Lcg` (tail sampling `draw < rate`, the plan's only new algorithm). Pure cores split from cells projecting emit onto `Cell<Option<T>>`; emit-only invalidation.
