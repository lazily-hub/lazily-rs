...
- `src/signal.rs` — eager `Signal` **derived construct** (not a core primitive; `Signal ≡ Slot.eager`) via `ctx.signal`; a memoized Slot plus a puller Effect, exposed on `Context`, `ThreadSafeContext`, and `AsyncContext`
...
- `tests/reactive_graph_conformance.rs` — reactive-graph disposal/teardown (`#lzspecedgeindex`) fixtures (lazily-spec/conformance/reactive-graph/`*.json`): edge detach in both directions, read-after-dispose, churn returns to baseline, recycled-id cleanliness, `ctx.scope()` teardown vs the fold of individual disposals, `disarm()`, cross-scope teardown hazard. First executor of that corpus in the family; asserts every assertion kind including `dependents_of`/`dependencies_of` (via `Context::dependent_count`/`dependency_count`) and carries a `KNOWN_DIVERGENCES` ledger, asserted in both directions, for assertions rs does not yet satisfy
